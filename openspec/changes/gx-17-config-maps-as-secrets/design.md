...
Coverage of the runnable checks: the kind flip for every converted resource (§1-§3), the Secret volume source for **every** pod template that mounts a converted map — golang, metrics, api, inference, celery (§4), the scope boundary folded into a RED-valid check (§5.1: `config-models-map` stays a ConfigMap while `config-yaml-map` became a Secret, so the combined check still fails on base), and **byte-identical payload parity** proven at runtime for **every** converted payload in **both** mirrors (§6, via `verify-payload-parity.sh`): the helper renders the base via `git archive origin/0.2.7`, asserts each HEAD render is a Secret, and diffs the payload block per resource (`config.yaml`, the five `config.py`, `credentials.json`) per mirror (`src/groundx` and `helm/`) — so parity is executed across the whole set, not asserted from a manual snapshot read. Verified GREEN on the branch and RED (every resource "HEAD is not kind: Secret") in an `origin/0.2.7` worktree. Only the docs task (§7.1) and hand-off/deferred items are `n/a`.
