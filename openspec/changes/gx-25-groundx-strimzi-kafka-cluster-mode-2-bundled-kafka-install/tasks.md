...
- [ ] 1.5 Wire the subchart's new `tests/` suite into the umbrella gate: add `helm unittest src/groundx/prereqs/kafka-cluster` to `.build/bin/validate-helm.sh` (next to the existing `helm unittest src/groundx` line) so the CI-enforced gate (`helm-tests.yml` → `.build/bin/validate-helm.sh`) actually runs it
...
- [ ] 2.1 `templates/services/stream-topics.yaml`: move the `KafkaTopic` CR to `apiVersion: kafka.strimzi.io/v1` only — `metadata.namespace` stays on `{{ include "groundx.ns" $ | quote }}`, unchanged
  check: helm unittest -f 'tests/stream_test.yaml' src/groundx
- [ ] 2.2 Re-baseline `tests/__snapshot__/stream_test.yaml.snap` for the `KafkaTopic` `apiVersion` change (`helm unittest -u src/groundx`, per AGENTS.md's documented regen command — never hand-edit the `.snap` file)
...
- [ ] 3.1 Mirror the group-1 edits (`cluster.yaml`, `nodepool.yaml`, `values.yaml`) into `helm/prereqs/kafka-cluster/`
...
- [ ] 3.2 Mirror the group-2 edit (`stream-topics.yaml`) into `helm/templates/services/stream-topics.yaml`
...
- [ ] 4.2 Add a one-line note next to the existing `stream:` block in `src/groundx/values.yaml` (and its `helm/` mirror) cross-referencing the README's namespace-pairing constraint, matching this file's existing commented-example style (e.g. the `existing:` block a few lines above)
  check: n/a — documentation only, no runtime behavior
- [ ] 4.3 Bump `src/groundx/prereqs/kafka-cluster/Chart.yaml` `version` from `0.1.1` to `0.2.0` (design.md decision 7) and mirror to `helm/prereqs/kafka-cluster/Chart.yaml`
