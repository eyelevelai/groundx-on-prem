...
- The first `helm upgrade` from either of those onto this fix **deletes** live objects in any
  existing `mode: ingest` install: the `ranker-api` and `ranker-inference` Deployments and
  Services, the three ranker ConfigMaps (`ranker-config-py`, `ranker-gunicorn-conf-py`,
  `ranker-supervisord-conf`), and the `ranker-model` PersistentVolumeClaim with its backing
  volume. No `helm.sh/resource-policy: keep` exists on any of these objects.
- The GPU model backing that PVC is re-downloadable from `upload.groundx.ai` at next pod start,
  so this is data cleanup rather than unrecoverable loss, but it is a real deletion an operator
  must be told about before they upgrade — **this needs a release note**, not just a changelog
  line. The hand-off task list already carries this into the PR body and the 0.2.7 release notes.
...
- Regenerate the three `src/groundx/tests/__snapshot__/*.snap` blocks that currently record the
  regression as golden (`api_test.yaml.snap`, `inference_test.yaml.snap`,
  `resources_test.yaml.snap`, each `extract.ingest:*` case) via `helm unittest -u src/groundx`.
  The regenerated diff must touch **only** those three `extract.ingest:*` blocks — any other block
  changing is itself a defect (the `mode: all` byte-identical constraint).
...
- Remove the now-dead `cluster.nodeLabels.gpuRanker: eyelevel-gpu-ranker` entry from
  `src/groundx/values/chainguard/values.yaml:55` and its `helm/` mirror — after this fix, nothing
  schedules to that node label under `mode: ingest`, so the entry only instructs an operator to
  provision a GPU node group for nothing.
...
- **Code:** `src/groundx/templates/_helpers/app/ranker-api.tpl`,
  ...
  `src/groundx/tests/ranker_test.yaml`; the three
  `src/groundx/tests/__snapshot__/{api,inference,resources}_test.yaml.snap` files (regenerated,
  never hand-edited); `.build/bin/validate-helm.sh`; `src/groundx/values/chainguard/values.yaml`
  and its `helm/` mirror.
...
- **Systems / data:** no schema or seed-data migration. The destructive-upgrade impact on a live
  `mode: ingest` install (Deployments, Services, three ConfigMaps, and the `ranker-model` PVC +
  backing volume) is a chart-render consequence surfaced above under Blast radius, not a database
