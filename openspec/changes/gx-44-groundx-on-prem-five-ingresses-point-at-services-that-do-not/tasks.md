...
- [ ] 1.1 In `src/groundx/templates/resources/ingress.yaml`, add `$backend` derived from
...
- [ ] 1.2 Use the same `$backend` at the legacy shape's backend site (`serviceName`).
      check: helm unittest src/groundx -f 'tests/ingress_test.yaml'
- [ ] 1.3 Mirror `resources/ingress.yaml` into `helm/` (byte-identical to `src/groundx/`) and add
...
- [ ] 2.1 In `src/groundx/templates/_helpers/app/ingress.tpl`'s `groundx.app.ingress` entry loop,
...
- [ ] 2.2 Mirror `_helpers/app/ingress.tpl` into `helm/` and add one
...
- [ ] 3.1 Run the repo's CI-parity validator end to end (lint, unit tests, snapshot guard,
