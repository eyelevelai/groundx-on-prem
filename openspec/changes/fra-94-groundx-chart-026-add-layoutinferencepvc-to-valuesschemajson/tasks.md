...
- [ ] 1.1 Create a scratch values file (e.g. `/tmp/fra-94-layout-pvc-test-values.yaml`, not
...
- [ ] 1.2 Run `helm lint src/groundx -f <scratch-values>` and `helm template src/groundx -f
...
- [ ] 2.1 In `src/groundx/values.schema.json`, insert the `pvc` property block into
...
- [ ] 2.2 Hand-mirror the identical `pvc` block into `helm/values.schema.json` at the structurally
      identical location (`layout.inference.properties`, between `nodeSelector` and `queue`).
- [ ] 2.3 Re-run the same `helm lint` / `helm template` commands from 1.2 with the same scratch
...
- [ ] 3.1 Run `diff src/groundx/values.schema.json helm/values.schema.json` — confirm **no
      output** (byte-identical), matching the repo's manual-mirror convention.
- [ ] 3.2 Run `helm lint src/groundx` (no custom values — chart defaults) — confirm it still
      passes, proving the change is additive and does not affect the default configuration.
- [ ] 3.3 Run `helm template src/groundx -f src/groundx/values/minikube/values.yaml` (the repo's
...
- [ ] 3.4 Run `helm unittest src/groundx` (the CI-enforced snapshot gate) — confirm all existing
...
- [ ] 3.5 Delete/discard the scratch values file used in tasks 1 and 2 (never commit throwaway
