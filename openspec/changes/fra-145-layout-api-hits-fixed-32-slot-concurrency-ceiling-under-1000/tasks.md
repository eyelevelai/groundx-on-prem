...
- [ ] 1.1 Add tolerant probe-timing rendering to the shared `src/groundx/templates/app/api.yaml`
...
- [ ] 3.1 Confirm an operator's `<service>.api.probe.{liveness,readiness}` override renders
...
- [ ] 4.1 Confirm the values schema rejects an undeclared key under `api.probe`/`api.probe.liveness`/
...
- [ ] 5.1 Manually sync the four touched files
...
- [ ] 6.1 Regenerate the affected `helm-unittest` snapshots
...
- [ ] 7.1 Run the repo's own CI-parity validator, covering both chart surfaces (lint, the full
