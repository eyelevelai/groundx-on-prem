...
- [x] 5.1 Append the explanatory comment to the `gpuRanker: eyelevel-gpu-ranker` line in
  ...
  check: HELM="${GX_ON_PREM_HELM:-helm}"; case "$("$HELM" version --short 2>/dev/null)" in v3.19.0|v3.19.0+*) ;; *) echo "helm ($HELM) is not v3.19.0 -- set GX_ON_PREM_HELM to a v3.19.0 binary" >&2; exit 1 ;; esac; grep -q 'gpuRanker: eyelevel-gpu-ranker  # schema requires all five nodeLabels keys' src/groundx/values/chainguard/values.yaml && grep -q 'gpuRanker: eyelevel-gpu-ranker  # schema requires all five nodeLabels keys' helm/values/chainguard/values.yaml && diff -q src/groundx/values/chainguard/values.yaml helm/values/chainguard/values.yaml >/dev/null && git diff --quiet -- src/groundx/values.schema.json helm/values.schema.json && "$HELM" lint src/groundx -f src/groundx/values/chainguard/values.yaml >/dev/null && "$HELM" lint helm -f helm/values/chainguard/values.yaml >/dev/null
