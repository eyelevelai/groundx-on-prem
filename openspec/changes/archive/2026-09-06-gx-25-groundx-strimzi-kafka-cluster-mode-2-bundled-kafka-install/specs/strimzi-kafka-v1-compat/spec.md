...
`src/groundx/values/chainguard/values.strimzi.operator.yaml` and its `helm/` mirror SHALL pin every Strimzi image `tag`/`tagPrefix` to `v0.50.1` (a dual-serving release that supports both `kafka.strimzi.io/v1beta2` and `kafka.strimzi.io/v1` and still supports Apache Kafka `4.0.x`; the exact `cgr.dev/eyelevel.ai` tag string is pending a human `crane ls` spot-check before the air-gapped publish), replacing the prior `v0.48.0` and keeping the values file's established `v`-prefixed tag convention. `src/groundx/values/chainguard/values.strimzi.cluster.yaml` and its `helm/` mirror SHALL leave `cluster.version` unset, replacing the prior `4.1.0` pin (0.50.1 supports Apache Kafka 4.0.0 through 4.1.1, so 4.1.0 is itself supported; the pin is dropped for consistency with the greenfield default, not because it is unsupported), matching the main v1 chart's greenfield default of leaving `cluster.version`/`cluster.metaVersion` unset.

Polarity: reject the prior air-gapped pins (the v1beta2-only `v0.48.0` operator image, and the `cluster.version: 4.1.0` override that diverges from the greenfield default) / accept the air-gapped path matching the already-shipped greenfield default (an unset cluster version, a `v1`-capable operator image).
