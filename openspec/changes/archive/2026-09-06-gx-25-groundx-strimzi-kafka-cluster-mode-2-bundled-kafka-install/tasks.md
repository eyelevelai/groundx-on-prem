...
- [x] 4.1 Bump every Strimzi image `tag`/`tagPrefix` in `src/groundx/values/chainguard/values.strimzi.operator.yaml` from `v0.48.0` to `v0.51.0` (keeping the file's established `v`-prefix convention; a registry check at apply time confirmed both `0.51.0` and `v0.51.0` are published Chainguard tags for this release); mirror the identical edit into `helm/values/chainguard/values.strimzi.operator.yaml`
...
`0.51.0` and `v0.51.0` are published tags for the target release, so the values files keep the
`v`-prefixed convention (`v0.51.0`); this builder had no `chainguard-pull-secret` credential and
