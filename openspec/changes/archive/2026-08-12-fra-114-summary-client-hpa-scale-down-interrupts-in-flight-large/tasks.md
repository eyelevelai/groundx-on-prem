...
- [x] 1.1 RED: add a `helm-unittest` schema-validation case asserting
...
- [x] 1.2 GREEN: add `"gracePeriod": { "type": "integer" }` to `summaryClient.replicas` in
      `src/groundx/values.schema.json`, keeping `additionalProperties: false`; re-run 1.1 and
      confirm it passes.
- [x] 1.3 RED: add a case to `src/groundx/tests/golang_test.yaml` (`set:
...
- [x] 1.4 GREEN: edit `src/groundx/templates/app/golang.yaml` to render
...
- [x] 1.5 RED: add a case to `src/groundx/tests/resources_test.yaml` (`set:
...
- [x] 1.6 GREEN: edit `src/groundx/templates/resources/config-yaml.yaml`'s `summaryServer`
...
- [x] 1.7 Add the "catches" sweep-consistency case (spec.md `queue-service-grace-period`):
...
- [x] 1.8 Add the below-margin edge case: `summaryClient.replicas.gracePeriod: 10` →
      `drainSeconds: 1` (floored, not negative); confirm it passes.
- [x] 1.9 Run `helm unittest src/groundx` scoped to `golang_test.yaml` and
...
- [x] 2.1 RED: add schema-validation cases (mirrors 1.1) for `process.replicas.gracePeriod`,
      `upload.replicas.gracePeriod`, `queue.replicas.gracePeriod`, and
      `preProcess.replicas.gracePeriod`; confirm each fails today.
- [x] 2.2 GREEN: add `gracePeriod` to the `process`/`upload`/`queue`/`preProcess` `replicas`
      blocks in `values.schema.json` (same shaped edit as 1.2, four more sites); re-run 2.1
      and confirm all pass.
- [x] 2.3 Add cases to `golang_test.yaml` asserting `terminationGracePeriodSeconds` renders
...
- [x] 2.4 RED: add cases to `resources_test.yaml` for each of `preProcessFileServer`/
      ...
      these four blocks).
- [x] 2.5 GREEN: edit the `preProcessFileServer`/`processFileServer`/`queueFileServer`/
...
- [x] 2.6 Add the "must not block" sweep-consistency case: all five services set distinct
...
- [x] 2.7 Repeat the "catches" case from 1.7 with a different single service isolated (e.g.
...
- [x] 3.1 Add an explicit case (at least one service) asserting that with no `gracePeriod`
...
- [x] 3.2 Run `helm unittest src/groundx` on the full `golang_test.yaml` and
...
- [x] 4.1 Run `helm unittest -u src/groundx` to regenerate
      `tests/__snapshot__/golang_test.yaml.snap` and `tests/__snapshot__/resources_test.yaml.snap`.
      (Ran; produced zero diff on both files — the groups 1–3 cases added above use
      `equal`/`matchRegex`/`notMatchRegex`/`failedTemplate` assertions, not `matchSnapshot`, so
      there is no new snapshot fixture to write. Separately confirmed `-u` on a fully-stashed
      base commit reproduces an unrelated, pre-existing snapshot-format diff across several
      other `_test.yaml.snap` files — a `helm-unittest` plugin-version formatting change
      (empty-manifest snapshot entries dropped), not caused by this change; left untouched.)
- [x] 4.2 Review the resulting `git diff` on both `.snap` files: confirm the only changes are
      ...
      spec.md's no-op requirement forbids. (Confirmed: zero-line diff on both files; see 4.1.)
- [x] 4.3 Copy the edited `src/groundx/values.schema.json` to `helm/values.schema.json`.
- [x] 4.4 Copy the edited `src/groundx/templates/app/golang.yaml` to
      `helm/templates/app/golang.yaml`.
- [x] 4.5 Copy the edited `src/groundx/templates/resources/config-yaml.yaml` to
      `helm/templates/resources/config-yaml.yaml`.
- [x] 4.6 `diff` each of the three `src/groundx/`↔`helm/` file pairs; confirm all three diffs
      are empty.

## 5. Verify (the repo's quality gates)

- [x] 5.1 `helm lint src/groundx` — clean.
- [x] 5.2 `helm unittest src/groundx` — green (this is the CI gate,
      `.github/workflows/helm-tests.yml`, and the only check that guards template changes).
- [x] 5.3 `helm template src/groundx -f src/groundx/values/minikube/values.yaml` — renders
      cleanly.
- [x] 5.4 Re-run the `src/groundx`↔`helm/` mirror `diff` from 4.6 one more time after all
