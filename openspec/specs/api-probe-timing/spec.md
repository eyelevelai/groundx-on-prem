# api-probe-timing Specification

## Purpose
Define the API-pod liveness/readiness probe-timing contract (explicit `timeoutSeconds`/
`failureThreshold`, operator overrides, schema validation, and `src/groundx`/`helm/` mirror
agreement) that replaces the fixed 32-slot concurrency ceiling's implicit probe defaults.
## Requirements
