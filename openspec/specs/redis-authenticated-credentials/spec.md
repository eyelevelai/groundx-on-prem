...
true AND its resolved credential is non-empty, mount it into the identity's StatefulSet, and launch
the container with `args: ["redis-server", "<mounted>/redis.conf"]` so the server starts with that
file. The rendered `redis.conf` begins with `include /etc/redis.conf` so the image's own default
configuration is preserved, then adds the credential, so the server actually enforces the credential
its clients already send (previously the chart-created Redis ran with no auth of its own, so a client
credential authenticated against an unauthenticated server). The credential value is wrapped in a
double-quoted, escaped `redis.conf` string so a password containing a space or reserved character
does not break `redis-server` startup.
...
- **THEN** the conf Secret's `redis.conf` contains an `include /etc/redis.conf` line followed by a
  `requirepass "<password>"` line, the cache StatefulSet mounts that Secret as a file volume, sets
  the container's `args` to `redis-server` followed by the mounted conf path, and carries a
  `config-hash` pod-template annotation derived from the conf Secret
...
- **THEN** the conf Secret's `redis.conf` contains `include /etc/redis.conf`, then `user default off`
  followed by `user <username> on ">password" ~* &* +@all`, and the metrics cache StatefulSet mounts
  it, sets `args` to `redis-server` followed by the mounted conf path, and carries the `config-hash`
  annotation
